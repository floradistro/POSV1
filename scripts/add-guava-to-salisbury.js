...
// Find Guava Candy product and Salisbury location
...
// Find Salisbury location by slug
async function findSalisburyLocation() {
  try {
    console.log('📍 Finding Salisbury location (slug: 28144)...');
    
    const response = await fetch(`${STORE_URL}/wp-json/wp/v2/mli_location?slug=28144&consumer_key=${WC_CONSUMER_KEY}&consumer_secret=${WC_CONSUMER_SECRET}`, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
    ...
    if (!response.ok) {
      console.log(`⚠️  WordPress API failed (${response.status}), trying to get all locations...`);
...
async function addSalisburyInventory(product, salisburyLocation) {
  try {
    ...
    // Remove any existing Salisbury location meta
    const cleanedMetaData = currentMetaData.filter(meta => 
      !meta.key.includes(`af_mli_location_stock_${salisburyLocationId}`) &&
      !meta.key.includes(`af_mli_location_price_${salisburyLocationId}`) &&
      !meta.key.includes('salisbury')
    );
    ...
    const newMetaData = [
      ...
      { key: 'salisbury_stock', value: '55' },
      { key: '_salisbury_stock', value: '55' },
      { key: 'salisbury_location_id', value: salisburyLocationId.toString() }
    ];
...
// Main execution
async function addGuavaToSalisbury() {
  try {
